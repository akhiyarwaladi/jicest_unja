@php
    ...
    $link = "ed-mono block py-1 text-[12px] tracking-[.16em] uppercase";
@endphp
...
<ul class="w-full flex flex-col md:flex-row md:items-center gap-2 md:gap-7">
    @foreach ($navList as $nav)
        @if ($nav['type'] == 'single')
            <li>
                <a href="{{ $nav['link'] }}"
                    class="{{$link}} {{$focus}} {{ in_array($title, $nav['inclusion']) ? 'text-[#047857]' : 'text-[rgba(11,27,20,0.70)] hover:text-[#0b1b14]' }}">
                    {{ $nav['name'] }}
                </a>
            </li>
        @endif
        @if ($nav['type'] == 'multiple')
            <li class="relative group" tabindex="0">
                <button
                    class="{{$link}} {{$focus}} flex items-center gap-1.5 {{ in_array($title, $nav['inclusion']) ? 'text-[#047857]' : 'text-[rgba(11,27,20,0.70)] group-hover:text-[#0b1b14]' }}">
                    {{ $nav['name'] }}
                        ...
                        @foreach ($nav['menu'] as $navmenu)
                            <li class="border-b border-[rgba(11,27,20,0.08)] last:border-b-0">
                                <a href="{{ $navmenu['link'] }}" @if ($navmenu['type'] == 'download') download @endif
                                    class="{{$focus}} block px-4 py-2.5 text-[13px] {{ in_array($title, $navmenu['inclusion']) ? 'text-[#047857]' : 'text-[rgba(11,27,20,0.70)] hover:bg-[#f3f2ec] hover:text-[#0b1b14]' }}">
                                    {{ $navmenu['name'] }}
                                </a>
                            </li>
                        @endforeach
    ...
    @else
        <li class="md:ml-3">
            <a href="/login"
                class="{{$focus}} ed-mono text-[12px] tracking-[.16em] uppercase py-1 px-1 text-[rgba(11,27,20,0.70)] hover:text-[#0b1b14]">Login</a>
        </li>
        <li>
